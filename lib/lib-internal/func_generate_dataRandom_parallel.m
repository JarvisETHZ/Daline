function [data, count] = func_generate_dataRandom_parallel(opt)
...
if strcmp(opt.load.distribution, 'normal')
    rf_load = opt.load.lowerRange + (opt.load.upperRange - opt.load.lowerRange) * randn(1, num_sample_redundant);
elseif strcmp(opt.load.distribution, 'uniform')
    rf_load = opt.load.lowerRange + (opt.load.upperRange - opt.load.lowerRange) * rand(1, num_sample_redundant);
end
...
% Get external parameters for parallel computation
idxPD = opt.idx.PD;
idxQD = opt.idx.QD;
idxVG = opt.idx.VG;
idxVA = opt.idx.VA;
...
% display
% clc
disp('Parallel computation starts for data generation')

% use acopf to generate the operating point for each scenario
parfor n = 1:num_sample_redundant
    
    % restore the test case
    mpc = mpc_backup;

    % change the load data 
    mpc.bus(:, idxPD) = loadSmallValue + loadAmplifyFactor * rf_load(n) * mpc.bus(:, idxPD);
    mpc.bus(:, idxQD) = loadSmallValue + loadAmplifyFactor * rf_load(n) * mpc.bus(:, idxQD);

    % change the voltage magnitude of generators, if requested
    mpc.gen(:, idxVG) = mpc.gen(:, idxVG) + rf_voltage(:, n);

    % change the voltage angle of the slack bus, if requested
    ...
    switch dataProgram
        ...
        case 'acpf'
            ...
            if changeGenerationACPF
                mpc.gen(:, idxPG) = genSmallValue + loadAmplifyFactor * rf_load(n) * mpc.gen(:, idxPG);
                mpc.gen(:, idxQG) = genSmallValue + loadAmplifyFactor * rf_load(n) * mpc.gen(:, idxQG);
            end
            % Run acpf
