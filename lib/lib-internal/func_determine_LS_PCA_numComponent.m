function Np = func_determine_LS_PCA_numComponent(X_train, Y_train, opt)
...
switch opt.PCA.rank
    ...
    case 0
        ...
        if isscalar(opt.PCA.PerComponent)
            ...
            if opt.PCA.parallel
                disp('Pallelization starts to tune the percentage of components in LS_PCA')
                best_PerComponent = func_tune_LS_PCA_parallel(X_train, Y_train, opt.PCA.PerComponent, opt.PCA.numFold, opt.PCA.fixCV, opt.PCA.fixSeed);
            else
                disp('Start to tune the percentage of components in LS_PCA')
                best_PerComponent = func_tune_LS_PCA(X_train, Y_train, opt.PCA.PerComponent, opt.PCA.numFold, opt.PCA.fixCV, opt.PCA.fixSeed);
            end
            Nx = size(X_train, 2);
