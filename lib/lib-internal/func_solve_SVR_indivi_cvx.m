function [Beta, success_all] = func_solve_SVR_indivi_cvx(X, Y, opt)
...
for j = 1 : Ny
    ...
    cvx_begin
        ...
        % Objective function
        J_j = 0.5 * sum(beta_j.^2) + opt.SVR.omega * sum(zi_j + zi_j_star);
        minimize(J_j)
    ...
    % Display progress
    pro_str = sprintf('SVR programming complete percentage: %3.1f', 100 * j / Ny);
