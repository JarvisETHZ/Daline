function best_PerComponent = func_tune_LS_PCA_parallel(X_train, Y_train, PerComponentCandidates, numFold, fixCV, fixSeed)
% This function performs cross-validation to select the best percentage of
% principal components for LS_PCA
% Inputs:
...
parfor idx = 1:numCandidates
    ...
    for i = 1:c.NumTestSets
        ...
        % Regression model on the training set based on LS solution
        Beta_PCA = ((X_train(train_idx, :) * D)' * (X_train(train_idx, :) * D)) \ ((X_train(train_idx, :) * D)' * Y_train(train_idx, :));
