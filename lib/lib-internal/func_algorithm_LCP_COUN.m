function model = func_algorithm_LCP_COUN(data, varargin)
% Linearly constrained programming without coupling constraints for Beta, as a comparison with the one with coupling constraints
%
%
...
model.predictorList = 'Fixed: Va, Vm';
...
X_train  = [data.train.Va, data.train.Vm];
...
X_test  = [data.test.Va, data.test.Vm];
...
[BetaPF, PF_cvx_status] = func_solve_LCP_CoupleNo(X_train, PF_train, opt);
[BetaPT, PT_cvx_status] = func_solve_LCP_CoupleNo(X_train, PT_train, opt);
[BetaQF, QF_cvx_status] = func_solve_LCP_CoupleNo(X_train, QF_train, opt);
[BetaQT, QT_cvx_status] = func_solve_LCP_CoupleNo(X_train, QT_train, opt);
