function [Beta, success_all] = func_solve_SVR_indivi_yalmip(X, Y, opt)
...
if opt.SVR.parallel
    % parallelly solving
    disp('Parallel computing starts for SVR')
    parfor j = 1 : Ny
        % Solve it using yalmip
        [beta_j, status] = func_solve_SVR_indivi_yalmip_j(X, Y(:, j), Nx, Ns, omega, epsilon, 0, options);
    ...
    for j = 1 : Ny
        ...
        % Display progress
        pro_str = sprintf('SVR complete percentage: %3.1f', 100 * j / Ny);
