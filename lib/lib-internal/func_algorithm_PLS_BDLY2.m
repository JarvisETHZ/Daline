function model = func_algorithm_PLS_BDLY2(data, varargin)
%  Training algorithm: partial least squares by bundling known and unknown 
%  variables. Aim is to handle bus-type variations, while verify the
%  technique of moving P_ref from "predictor" to "response". Here P_ref is
%  a predictor in Y2_test, while in func_algorithm_PLS_BDL, P_ref is a
%  response in Y1_test. The comparison between them two would tell us the
%  significance of moving P_ref. 
%
...
X2_train = [X.train.Va, X.train.Vm]; 
Y1_train = [Y.train.Va, Y.train.Vm, Y.train.PF, Y.train.PT, Y.train.QF, Y.train.QT];
Y2_train = [Y.train.P,  Y.train.Q];
X1_test  = [X.test.P,   X.test.Q]; 
X2_test  = [X.test.Va,  X.test.Vm]; 
Y1_test  = [Y.test.Va,  Y.test.Vm, Y.test.PF, Y.test.PT, Y.test.QF, Y.test.QT];
Y2_test  = [Y.test.P,   Y.test.Q];
...
[model.error, model.yPrediction, model.yTrue] = func_split_error_BDLY2(Y, error, [Y1_prediction, Y2_prediction], [Y1_test, Y2_test]);
