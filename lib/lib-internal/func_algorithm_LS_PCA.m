function model = func_algorithm_LS_PCA(data, varargin)
%  Training algorithm: Least squares with principal component analysis

%% Start

% Specify the algorithm
model.algorithm = 'LS_PCA';
...
Np = func_determine_LS_PCA_numComponent(X_train, Y_train, opt);
model.Np = Np;
...
Beta_PCA = ((X_train * D)' * (X_train * D)) \ ((X_train * D)' * Y_train);
...
% Record the det of XDTXD
model.det_XDTXD = det((X_train * D)' * (X_train * D));
